...
class FhevmEnvironmentPaths {
    ...
    get cachePrecompiledFhevmHostContractsAddressesJson() {
    ...
    get zamaFheRelayerSdkDir() {
        return path.dirname(this._resolveFromConsumer(path.join(constants_1.default.ZAMA_FHE_RELAYER_SDK_PACKAGE.name, "package.json")));
    }
    resolveFhevmHostContractsArtifactRootDir() {
        let modulePath = path.resolve(path.join("artifacts", constants_1.default.FHEVM_HOST_CONTRACTS_PACKAGE.name));
        if (!fs.existsSync(modulePath)) {
            modulePath = path.join(constants_1.default.FHEVM_HOST_CONTRACTS_PACKAGE.name, "artifacts");
        }
        return modulePath;
    }
    resolveFhevmHostContractsArtifactPath(contractName) {
    ...
    async getFhevmHostContractsArtifact(contractName) {
